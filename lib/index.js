...
function findProfileRoot() {
  ...
  while (dir !== resolve(dir, '..')) {
    ...
    if (existsSync(pkgPath)) {
      ...
      // 修复：使用 startsWith 替代字面量通配符
      if (pkg.name === 'dsh-profile-web' || pkg.name?.startsWith('dsh-profile-') || pkg.dsh?.profile) {
        return dir;
      }
    }
    dir = resolve(dir, '..');
