...
1. **Startup scan**: When loaded as a DSH plugin, it scans the current profile's `package.json` dependencies and the actual `node_modules` installation to detect:
   ...
   - `dsh.profile.bundles` vs `dependencies` duplication (normal, not reported)
...
|------|----------|-------------|
| Duplicate declaration in package.json | ❌ Error | Same plugin name declared with multiple sources |
...
| dsh.bundles vs dependencies duplication | Info | Normal, not reported |
...
3. **Detection logic**:
   - Parse `dependencies` / `devDependencies` / `optionalDependencies` from `package.json`
